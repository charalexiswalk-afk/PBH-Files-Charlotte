...
The published curves are reconstructed using the tabulated HyRec luminosities, effective temperatures, and the factor 1 + gamma^(1/3) used in the original
numerical implementation. The zero-velocity curves are then calculated by setting the effective gas temperature equal to T_gas and recomputing the PBH
accretion rate, luminosity, and thermal-feedback quantity for both the collisional-ionization and photoionization limits.
This comparison isolates the effect of baryon-dark matter relative velocities on the Figure 9 thermal-feedback estimate.
"""

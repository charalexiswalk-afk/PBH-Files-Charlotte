...
for M in masses:
    ...
    for collisional, linestyle in (
        ...
        peak_index = int(np.argmax(zero_v))
        branch = "coll" if collisional else "photo"

        print(
            f"ZERO-v CHECK | M={M:g}, {branch:5s}: "
            f"peak={zero_v[peak_index]:.3e}, "
            f"z={z[peak_index]:.3e}"
        )
...
output_path = (
    ...
    / "figure9_published_vs_zero_velocity.png"
)

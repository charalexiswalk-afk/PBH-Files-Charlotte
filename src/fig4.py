...
def lambda_gamma(gamma):
    ...
    return lambda_ad + (lambda_iso - lambda_ad) * (
        gamma**2 / (88.0 + gamma**2)
    )**0.22

def lambda_beta(beta):

    return np.exp((9.0/2.0)/(3.0 + beta**0.75)) / (
        np.sqrt(1.0 + beta) + 1.0
    )**2

def lambda_total(gamma, beta):

    return lambda_gamma(gamma) * lambda_beta(beta) / lambda_iso
...
for Mfac, label, color in zip(masses, labels, colors):
    ...
    axes[0].loglog(z, lam, color=color, linewidth=2, label=label)
    axes[1].loglog(z, mdot, color=color, linewidth=2, label=label)

axes[0].axhline(lambda_iso, color="black", linestyle=":", linewidth=1)
axes[0].axhline(lambda_ad, color="black", linestyle=":", linewidth=1)
